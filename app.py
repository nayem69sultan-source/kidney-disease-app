...
# Form for input sliders
with st.form(key='input_form'):
    user_input = []
    for feature in feature_names:
        min_val, max_val, default_val = feature_ranges[feature]
        if feature == "age":
            value = st.slider(f"{feature}", int(min_val), int(max_val), int(default_val))
        else:
            value = st.slider(f"{feature}", float(min_val), float(max_val), float(default_val))
